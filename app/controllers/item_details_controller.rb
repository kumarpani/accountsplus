class ItemDetailsController < ApplicationController
  ...
  def print
    ...
    respond_to do |format|
      format.pdf do
        pdf = PrintQuotation.new(@quotation.id, params[:unit_price], params[:bank], params[:seal], params[:gst])
        send_data pdf.render, filename: @quotation.get_pdf_name,
  ...
  private
    ...
    def item_detail_params
      params.require(:item_detail).permit(:particulars, :unit_price, :quantity, :days, :item_group_name, :saccode)
    end
end
